    ...
    private void InteractioRaycast()
    {
        ...
        if (Physics.Raycast(playerCamera.transform.position, playerCamera.transform.forward, out hitInfo, interactionDistance, layerMask))
        {
            ...
            if (Input.GetKeyDown(KeyCode.E))
            {
                ...
                if (hitInfo.transform.name == "Apple(C)")
                {
                    CountingApple.appleCount += 1;
                }
                else if (hitInfo.transform.name == "Pear(C)")
                {
                    CountingPear.pearCount += 1;
                }
                else if (hitInfo.transform.name == "Barrel(C)")
                {
                    CountingCarrot.carrotCount += 1;
                }
            }
        }
    }
